...
class TestActions(object):
	...
	def test_tilt(self):
		...
		assert parser.from_json_data({'action' : 'tilt( button(KEY_D), button(KEY_U), button(KEY_L), button(KEY_R))'}).actions[3].button == Keys.KEY_R
	...
	def test_dpad(self):
		a = parser.from_json_data({'dpad': [{'action': 'button(KEY_A)'}, {'action': 'button(KEY_B)'}, {'action': 'button(KEY_C)'}, {'action': 'button(KEY_D)'}]})
		assert isinstance(a, DPadAction)
		for sub in a.actions:
			assert isinstance(sub, ButtonAction)

	def test_ring(self):
		a = parser.from_json_data({'ring': {'radius': 0.3, 'outer': {'dpad': [{'action': 'button(KEY_A)'}, {'action': 'button(KEY_B)'}, {'action': 'button(KEY_C)'}, {'action': 'button(KEY_D)'}]}, 'inner': {'X': {'action': 'axis(ABS_X)'}, 'Y': {'action': 'axis(ABS_Y)'}}}})
	...
	def test_dpad8(self):
		a = parser.from_json_data({'dpad': [{'action': 'button(KEY_A)'}, {'action': 'button(KEY_B)'}, {'action': 'button(KEY_C)'}, {'action': 'button(KEY_D)'}, {'action': 'button(KEY_E)'}, {'action': 'button(KEY_F)'}, {'action': 'button(KEY_G)'}, {'action': 'button(KEY_H)'}]})
		print(a)
		print(a.actions)
		assert isinstance(a, DPadAction)
		for sub in a.actions:
	...
	def test_XY(self):
		a = parser.from_json_data({'X': {'action': 'axis(ABS_X)'}, 'Y': {'action': 'axis(ABS_Y)'}})
