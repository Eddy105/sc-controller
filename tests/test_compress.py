		...
		for cls in Action.ALL.values():
			...
			if hasattr(cls, "set_speed"):
				assert cls.COMMAND in CASES, (
					"%s supports setting sensitivity, but there is no test case it" % cls.COMMAND)
			...
			if hasattr(cls, "set_haptic"):
				assert cls.COMMAND in CASES, (
					"%s supports feedback, but there is no test case it" % cls.COMMAND)
