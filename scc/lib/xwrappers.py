...
class XWindowAttributes(Structure):
	_fields_ = [
		...
		('your_event_mask', c_long), ('do_not_propagate_mask', c_long), ('map_installed', c_bool),
		('screen', c_void_p)
	]
...
query_pointer.argtypes = [c_void_p, XID, POINTER(XID), POINTER(XID), POINTER(c_int), POINTER(c_int), POINTER(c_int), POINTER(c_int), POINTER(c_uint)]
...
get_window_property.argtypes = [c_void_p, XID, Atom, c_long, c_long, c_bool, Atom, POINTER(Atom), POINTER(Atom), POINTER(c_ulong), POINTER(c_ulong), POINTER(c_void_p)]
...
def get_mouse_pos(dpy, relative_to=None):
	...
	mask = c_uint()
	query_pointer(dpy, relative_to, byref(root_return), byref(child), byref(x), byref(y), byref(child_x), byref(child_y), byref(mask))
...
def get_window_prop(dpy, window, prop_name, max_size=2):
	...
	prop = c_void_p()
	if SUCCESS == get_window_property(dpy, window, prop_atom, 0, max_size, False, ANYPROPERTYTYPE, byref(type_return), byref(format_return), byref(nitems), byref(bytes_after), byref(prop)):
